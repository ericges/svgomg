...
A rule is a predicate over `getSettings()` **gated on what the document contains**. `src/js/svgo-worker/document-features.js` supplies the three facts — `hasStyleElement`, `hasScripts`, `hasMask` — as one more visitor in a pass that was happening anyway, and `SvgFile` carries them so a cached result keeps its own. **The gate is the whole reason the feature is bearable**: without it the default settings would show five permanent notices on every file, including the ones where nothing is affected.

`MainController._updateDocumentFacts()` is where it gets subtle: the two constructs are read off **different documents**, and both choices are forced by pipeline order.

- `hasScripts` and `hasMask` come from the **input** (`wrapOriginal`), because `removeScripts` runs *after* every plugin that backs off for a script. On a single pass it's too late to help — only a second `multipass` pass sees a script-free document, which is exactly what the message offers as the fix. That half is settings-derived, and `test/setting-notes.test.js` pins the ordering it assumes against `panelOrder`.
- `hasStyleElement` comes from the **result** (`process`, extractor last in the array), because every subject runs after the Styles block: a stylesheet still in the output was there for all of them. No settings condition rides on top, and none could — "Remove style elements" always clears it, but "Inline into elements" clears it only when *every* rule turned out to be inlinable (SVGO inlines a rule only if its selector matches exactly one element and it isn't behind a non-screen media query). Measuring the output is what lets a successful inlining silence the notice and an unsuccessful one keep it; `test/document-features.test.js` pins both cases.

The extractor resets its flags on `root.enter`, so under `multipass` the facts describe the **last** pass rather than "seen at some point" — a stylesheet cleared on pass one is genuinely gone for everything that runs on pass two.

`collectNotes()` returns nothing at all before the first file, or while "Show original" is on — nothing is being optimised then. A checkbox inside a collapsed stage block has no visible row, so its notice is hosted by the block's select instead (that's how the `minifyStyles` one is reachable at all). The `idPrefix` rule is the odd one out: it needs no document, and it shares `normalizeIdPrefix` with the worker so the notice and the pipeline agree on what a usable prefix is. Messages quote controls by their panel label; `setting-notes.js` exports those strings and `test/build-smoke.test.js` checks each still exists in the built markup, so renaming an option can't leave the advice pointing at nothing.
