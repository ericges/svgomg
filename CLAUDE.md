...
| `minifyStyles` | a script (usage-based pruning only) | there is a stylesheet with rules |
| `current-color-styles` | a `<mask>` (stylesheets only; attributes still convert) | there is a stylesheet |
| `removeUselessStrokeAndFill` | **any** `<style>` element, empty included, or a script | always — it returns nothing at all |
| `removeHiddenElems` | a `<style>` with children or a script, and **only the deferred sweep** | there are non-rendering elements or transparent paths |
| `moveElemsAttrsToGroup` | **any** `<style>` element | there is a `<g>` with more than one child |

Two of those splits are easy to get wrong and each has cost a false notice: an empty `<style>` really does stop the second pair and really doesn't stop the first, and `removeHiddenElems` keeps removing zero-sized and `display:none` elements while its deferred half is switched off — so its message says which half. The per-subject gate in the right-hand column is what keeps a notice from claiming something was overruled when there was nothing to overrule.
...
Two conditions in `setting-notes.js` are still settings-derived, and both **anticipate the result that is already being computed**: the panel re-renders the instant a control moves, so without them it would go on advising the option the user just picked until the worker came back. "Remove style elements" runs ahead of every subject but `minifyStyles`, and "Remove scripts" + "Multipass" clears a script for the last pass. Both are claims about plugin order, pinned against `panelOrder` in `test/setting-notes.test.js`.
...
Two approximations are deliberate, both in `removeHiddenElems`: `hasDeferredHiddenCandidate` doesn't run reference analysis (so `removeHiddenElems` can be reported as held back where every candidate was referenced anyway — the message claims SVGO stopped looking, not that anything would have gone), and a transparent path is detected from its `opacity` attribute or `style`, not a computed cascade.
