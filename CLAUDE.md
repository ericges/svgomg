...
`src/test-svgs/kitchen-sink.svg` is the one fixture written *for* the settings panel rather than as artwork: a labelled test card carrying every construct the exposed optimisations act on, so that toggling any single control visibly changes the output. `test/kitchen-sink.test.js` reads it out of `src/` (no build needed) and runs each `config.json` plugin over it alone, failing with the plugin's name if one turns out to be a no-op — so trimming a tile is caught rather than silently narrowing what a hand test can reach. Two carve-outs it encodes, both worth knowing before editing the file:
