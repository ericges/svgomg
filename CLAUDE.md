...
Several modules exist only so that logic is reachable outside a browser, and should stay that way: `src/js/svgo-worker/build-plugins.js`, `dimensions.js`, `ensure-dimensions.js`, `current-color-styles.js` and `id-prefix.js` (the worker entry point exports nothing and ends in a `self.onmessage` assignment), `src/js/page/ui/preview-size.js` and `src/js/page/ui/setting-stages.js` (`svg-output.js` and `settings.js` reach for `document`), and `src/js/page/migrate-settings.js`. Each is a sibling of its only consumer, which changes no output filename — bundles are named after their *directory*.
