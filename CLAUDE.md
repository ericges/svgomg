...
- **Both precision sliders need a witness `convertPathData` cannot swallow.** Short exact fractions round away by precision 2 and then sit still, so the float slider gets a path with seven decimals on every coordinate. Transform precision is harder: on a `<g>` nothing survives, because `moveGroupAttrsToElems`, `collapseGroups` and `convertPathData` bake the transform into the path data at *float* precision first. The witness is therefore a `<use>` with a fractional `scale()` — one of the few things a transform can't be applied into. Remove either and that slider becomes a control the fixture can't demonstrate.
